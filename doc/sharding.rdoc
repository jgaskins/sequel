= Read-Only Replicas/Writable Primary and Database Sharding

Sequel has support for read only replica databases
with a writable primary database, as well as database sharding (where you can
...
=== Single Read-Only Replica, Single Primary
...
the symbol name defined in the connect options. For example:
  ...
  # Force the SELECT to run on the primary
  DB[:users].server(:default).all

  # Force the DELETE to run on the read-only replica
  DB[:users].server(:read_only).delete

=== Multiple Read-Only Replicas, Single Primary

Let's say you have 4 replica database servers with names replica_server0,
...
=== Multiple Read-Only Replicas, Multiple Primaries

This involves the same basic idea as the multiple replicas, single primary, but
it shows that the primary database is named :default.  So for 4 primaries and
4 replicas:
